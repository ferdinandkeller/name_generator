...
This package provides a simple name generator for Python. It is based on the Name Generator from the Moby Project.
"""

from random import randint

__version__ = "1.0.0"
__author__ = "Ferdinand Keller"
...
def random_ajective() -> str:
    adjective_index = randint(0, len(adjectives) - 1)
    return adjectives[adjective_index]


def random_lastname() -> str:
    lastname_index = randint(0, len(lastnames) - 1)
    return lastnames[lastname_index]


def random_pair() -> (str, str):
    adjective = random_ajective()
    lastname = random_lastname()
    if adjective == "boring" and lastname == "wozniak":  # Steve Wozniak is not boring
        return random_pair()
    else:
        return (adjective, lastname)


def random_name(separator: str = "_") -> str:
    (adjective, lastname) = random_pair()
    return f"{adjective}{separator}{lastname}"
